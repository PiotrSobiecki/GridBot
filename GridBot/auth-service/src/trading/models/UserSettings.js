...
export class UserSettings {
  constructor(data = {}) {
    ...
    this.wallet = this._parseJson(data.wallet || data.wallet_currencies || '[]');
    ...
    if (this.orders.length === 0) {
      this.orders = [{
        ...
        buySwingPercent: [
          ...
          { minTrend: 2, maxTrend: 2, value: 0.2 },
          { minTrend: 3, maxTrend: 3, value: 0.5 },
          { minTrend: 4, maxTrend: 999, value: 1 }
        ],
        sellSwingPercent: [
          { minTrend: 0, maxTrend: 1, value: 0.1 },
          { minTrend: 2, maxTrend: 2, value: 0.2 },
  ...
  static findOne({ walletAddress }) {
    const row = db.prepare('SELECT * FROM user_settings WHERE wallet_address = ?').get(walletAddress?.toLowerCase());
